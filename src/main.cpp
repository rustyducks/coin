...
int main(int, char**) {
    ...
    rd::UDPDucklink udpClient("127.0.0.1", 9870);
    rd::UDPDucklink udpClientAnatidae("127.0.0.1", 3456);
    ...
    while (true) {
        ...
        if (dt > 0.1) {
            ...
            udpClient.sendSpeedJson(speedCmd, "speed_cmd");
        ...
        if (dtSimu > 0.02) {
            lastSimu = now;
            robotSpeed = speedCmd * 0.6 + robotSpeed * 0.4;
            ...
            udpClient.sendSpeedJson(robotSpeed, "robot_speed");
            udpClient.sendPointOrientedJson(robotPose);
